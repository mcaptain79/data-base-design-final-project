...
def booksSet():
    mySet = set()
    query = ('select bookId,bookNumber from book')
    cursor.execute(query)
    for (bookId,bookNumber) in cursor:
        mySet.add(bookId+'-'+str(bookNumber))
    return mySet
#saving results in a dictionary
books = booksSet()
print(books)
#function below is for publiser names
def publisherSet():
    mySet = set()
...
def ordinaryBorrowBook(nationalCode):
    ...
    if bookId+'-'+str(bookNumber) in nonReturnedSet:
        ...
        return
    if bookId+'-'+str(bookNumber) not in books:
        ...
        query3 = ('update ordinary_account set balance = balance-%s where nationalCode = %s')
        cursor.execute(query3,(cost,nationalCode))
        myDB.commit()
...
def instructorBorrowBook(nationalCode):
    ...
    if bookId+'-'+str(bookNumber) not in books:
        ...
        cursor.execute(query2,(nationalCode,len(histories)+1))
        histories.add(len(histories)+1)
        query3 = ('update instructor_account set balance = balance-%s where nationalCode = %s')
        cursor.execute(query3,(cost,nationalCode))
        myDB.commit()
        print('borrow done successfully')
...
def studentBorrowBook(nationalCode):
    ...
    if bookId+'-'+str(bookNumber) not in books:
        ...
        cursor.execute(query2,(nationalCode,len(histories)+1))
        histories.add(len(histories)+1)
        query3 = ('update student_account set balance = balance-%s where nationalCode = %s')
        cursor.execute(query3,(cost,nationalCode))
        myDB.commit()
        print('borrow done successfully')
...
def signInOrUp():
    ...
    if choice == 1:
        ...
        print('invalid input')
        signInOrUp()
#functions below are for admin actions
def search_users():
    print('1)by user name\n2)by family')
    choice = int(input('enter how you want to search: '))
    if choice == 1:
        userName = input('enter userName: ')
        query1 = ('select * from student where userName = %s')
        query2 = ('select * from instructor where userName = %s')
        query3 = ('select * from ordinary where userName = %s')
        ...
        query1 = ('select * from student where family = %s')
        query2 = ('select * from instructor where family = %s')
        query3 = ('select * from ordinary where family = %s')
        print('search results')
        cursor.execute(query1,(family,))
        for i in cursor.fetchall():
            print(i)
        cursor.execute(query2,(family,))
        for i in cursor.fetchall():
            print(i)
        cursor.execute(query3,(family,))
        for i in cursor.fetchall():
            print(i)
    else:
        print('invalid input')
#functions below are for deleting
#for student
def studentDel():
    nationalCode = ''
    userName = input('enter user name you want to delete: ')
    query1 = ('select nationalCode from student where userName = %s')
    cursor.execute(query1,(userName,))
    for i in cursor:
        nationalCode = i[0]
    query2 = ('delete from student_account_history where nationalCode = %s')
    cursor.execute(query2,(nationalCode,))
    query3 = ('delete from student_account where nationalCode = %s')
    cursor.execute(query3,(nationalCode,))
    query4 = ('delete from student_address where userName = %s')
    cursor.execute(query4,(userName,))
    query5 = ('delete from student_phoneNum where userName = %s')
    cursor.execute(query5,(userName,))
    query6 = ('delete from student where userName = %s')
...
def instructorDel():
    nationalCode = ''
    userName = input('enter user name you want to delete: ')
    query1 = ('select nationalCode from instructor where userName = %s')
    cursor.execute(query1,(userName,))
    for i in cursor:
        nationalCode = i[0]
    query2 = ('delete from instructor_account_history where nationalCode = %s')
    cursor.execute(query2,(nationalCode,))
    query3 = ('delete from instructor_account where nationalCode = %s')
    cursor.execute(query3,(nationalCode,))
    query4 = ('delete from instructor_address where userName = %s')
    cursor.execute(query4,(userName,))
    query5 = ('delete from instructor_phoneNum where userName = %s')
    cursor.execute(query5,(userName,))
    query6 = ('delete from instructor where userName = %s')
    cursor.execute(query6,(userName,))
    myDB.commit()
    print('deletion done successfully')
#for ordinary people
def ordinaryDel():
    nationalCode = ''
    userName = input('enter user name you want to delete: ')
    query1 = ('select nationalCode from ordinary where userName = %s')
    cursor.execute(query1,(userName,))
    for i in cursor:
        nationalCode = i[0]
    query2 = ('delete from ordinary_account_history where nationalCode = %s')
    cursor.execute(query2,(nationalCode,))
    query3 = ('delete from ordinary_account where nationalCode = %s')
    cursor.execute(query3,(nationalCode,))
    query4 = ('delete from ordinary_address where userName = %s')
    cursor.execute(query4,(userName,))
    query5 = ('delete from ordinary_phoneNum where userName = %s')
    cursor.execute(query5,(userName,))
    query6 = ('delete from ordinary where userName = %s')
    cursor.execute(query6,(userName,))
    myDB.commit()
    print('deletion done successfully')
#function below is for choosing delete from which group
def deleteChoice():
    print('1)student\n2)instructor\n3)ordinary')
    choice = int(input('who you want to delete: '))
    if choice == 1:
        studentDel()
    elif choice == 2:
        instructorDel()
        ...
        print('invalid input')
#function below is for adding a book
def addBook():
    bookId = input('enter bookId: ')
    bookNumber = int(input('enter bookNum: '))
    if bookId+'-'+str(bookNumber) in books:
        ...
        if publisherName not in publishers:
            print('invalid publisher name')
        else:
            query1 = ('insert into book (bookId,bookNumber,title,field,pages,price,publisherName) values (%s,%s,%s,%s,%s,%s,%s)')
            cursor.execute(query1,(bookId,bookNumber,title,field,pages,price,publisherName))
            myDB.commit()
...
def adminPannel():
    while True:
        ...
        if choice == 1:
            ...
            hquery = ('select * from history order by returnedDate')
            ...
            print('invalid input')
#this func is for admin sign in
def adminSignIn():
    userName = ''
    password = ''
    while True:
